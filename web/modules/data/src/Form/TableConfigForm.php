...
class TableConfigForm extends EntityForm {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    // Enforce MySQL constraint: only one auto-increment (serial) column and it
    // must be part of the primary key.
    $rows = $form_state->getValue('table_schema') ?: [];
    $serial_count = 0;
    $serial_index = NULL;
    $primary_count = 0;
    foreach ($rows as $i => $row) {
      if (!empty($row['type']) && $row['type'] === 'serial') {
        $serial_count++;
        $serial_index = $i;
        if (empty($row['primary'])) {
          $form_state->setErrorByName("table_schema][$i][primary", $this->t('A serial field must be set as a primary key.'));
        }
      }
      if (!empty($row['primary'])) {
        $primary_count++;
    ...
    if ($serial_count > 1) {
      $form_state->setErrorByName('table_schema', $this->t('Only one serial field is allowed per table.'));
    }
    // If a serial field exists, it must be the only primary key.
    if ($serial_count === 1 && $primary_count !== 1) {
      $form_state->setErrorByName('table_schema', $this->t('When a serial field is used, it must be the only primary key.'));
    }
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        \Drupal::messenger()->addStatus($this->t('Created the %label Data Table.', [
          '%label' => $data_table_config->label(),
        ]));
        break;

      default:
        \Drupal::messenger()->addStatus($this->t('Saved the %label Data Table.', [
          '%label' => $data_table_config->label(),
        ]));
    }
    $form_state->setRedirectUrl($data_table_config->toUrl('collection'));
  }
  ...
  protected function fieldForm($index, $required = FALSE) {
    ...
    if (!$this->entity->isNew()) {
      if (!empty($this->entity->meta['relation'][$current['name']])) {
        $relation_label = $this->entity->meta['relation'][$current['name']]['base_table'] . '.' .
            $this->entity->meta['relation'][$current['name']]['base_field'];
        $form['relation']['#value'] = 'old';
      }
      else {
        $relation_label = $this->t('none');
      }
      $form['relation'] = [
        '#type' => 'link',
        ...
        '#url' => Url::fromRoute(
          ...
          ['table_name' => $this->entity->id(), 'field_name' => $current['name']]
        ),
      ];
    }
    $form['date'] = [
      '#type' => 'select',
      '#options' => [
        '' => $this->t('Not a date field'),
        'date_iso' => $this->t("ISO date: 'Y-m-dTH:i:s'"),
        'date' => $this->t('Unix timestamp'),
      ],
