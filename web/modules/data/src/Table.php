...
use Drupal\Core\Database\SchemaObjectExistsException as CoreSchemaObjectExistsException;
use Drupal\Core\Database\SchemaObjectDoesNotExistException as CoreSchemaObjectDoesNotExistException;
/**
...
class Table {
  ...
  public function __construct($name) {
    ...
    $this->name = Database::getConnection()->escapeTable($name);
  ...
  public function create($table_schema) {
    ...
    // physically exist.
    if (!_data_load_table($this->name, TRUE) && !Database::getConnection()->schema()->tableExists($this->name)) {

      // Create table.
      try {
        Database::getConnection()->schema()->createTable($this->name, $table_schema);
      }
      catch (CoreSchemaObjectExistsException $e) {
        \Drupal::messenger()->addError(t('Error creating table.'));
        return FALSE;
      }
      ...
      // on DataTable.
      if (\Drupal::moduleHandler()->moduleExists('schema')) {
      ...
      Database::getConnection()->merge('data_tables')
        ->key(['name' => $this->name])
        ->fields($table)
        ->execute();

      // Clear caches.
      if (function_exists('drupal_get_schema')) {
      ...
      if (\Drupal::moduleHandler()->moduleExists('data_ui')) {
  ...
  public function adopt() {
    if ($this->defined() || !\Drupal::moduleHandler()->moduleExists('schema')) {
      return FALSE;
    }

    $schema = schema_dbobject()->inspect('default', $this->name);
    if (isset($schema[$this->name])) {
      $table = array(
        'name' => $this->name,
      ...
      $result = Database::getConnection()->merge('data_tables')
        ->key(['name' => $this->name])
        ->fields($table)
        ->execute();
      if ($result) {
  ...
  public function disown() {
    ...
    $num_deleted = Database::getConnection()->delete('data_tables')
  ...
  public function addField($field, $spec) {
    try {
      Database::getConnection()->schema()->addField($this->name, $field, $spec);
  ...
  public function addIndex($field) {
    ...
    if ($schema['fields'][$field]) {
      ...
      try {
        Database::getConnection()->schema()->addIndex($this->name, $field, $index);
      }
      catch (CoreSchemaObjectExistsException $e) {
  ...
  public function dropIndex($field) {
    $success = Database::getConnection()->schema()->dropIndex($this->name, $field);
    if ($success) {
      $schema = $this->table_schema;
  ...
  public function addUniqueKey($field) {
    ...
    if ($schema['fields'][$field]) {
      $index = data_get_index_definition($field, $schema['fields'][$field]);
      try {
        $success = Database::getConnection()->schema()->addUniqueKey($this->name, $field, $index);
      }
      catch (CoreSchemaObjectExistsException $e) {
  ...
  public function dropUniqueKey($field) {
    $success = Database::getConnection()->schema()->dropUniqueKey($this->name, $field);
    if ($success) {
      $schema = $this->table_schema;
  ...
  public function addPrimaryKey($fields) {
    ...
    try {
      Database::getConnection()->schema()->addPrimaryKey($this->name, $fields);
    }
    catch (CoreSchemaObjectExistsException $e) {
  ...
  public function dropPrimaryKey() {
    Database::getConnection()->schema()->dropPrimaryKey($this->name);
  ...
  public function changeField($field, $spec) {
    ...
    try {
      Database::getConnection()->schema()->changeField($this->name, $field, $field, $spec);
    }
    catch (CoreSchemaObjectDoesNotExistException $e) {
  ...
  public function dropField($field) {
    $success = Database::getConnection()->schema()->dropField($this->name, $field);
  ...
  public function drop() {
    if ($this->export_type == EXPORT_IN_DATABASE) {
      if (Database::getConnection()->schema()->tableExists($this->name)) {
        Database::getConnection()->schema()->dropTable($this->name);
      }
      ...
      Database::getConnection()->delete('data_tables')
        ->condition('name', $this->name)
        ->execute();
      $this->title = '';
